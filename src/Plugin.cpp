...
static void LogDebug(const std::string &msg) {
  std::ofstream ofs("d:\\software\\bdshare_plugin_debug.log", std::ios::app);
  ofs << msg << "\n";
}
...
// One null-sentinel entry; gFunctionTableSize=0 tells AmiBroker: no AFL
// functions.
...
static char g_statusShort[32] =
    "----"; // matches PluginStatus.szShortMessage[32]
...
PLUGINAPI int GetPluginStatus(struct PluginStatus *status) {
  ...
  if (g_statusCode == STATUS_OK) {
    status->clrStatusColor = RGB(0, 200, 0);
  } else if (g_statusCode == STATUS_WAIT) {
    status->clrStatusColor = RGB(255, 165, 0);
  } else if (g_statusCode == STATUS_CONNECTING) {
    status->clrStatusColor = RGB(0, 120, 215);
  } else if (g_statusCode == STATUS_ERROR) {
    status->clrStatusColor = RGB(220, 0, 0);
  } else {
    status->clrStatusColor = RGB(180, 180, 180);
  }

  strcpy_s(status->szShortMessage, sizeof(status->szShortMessage),
           g_statusShort);
...
PLUGINAPI int Configure(LPCTSTR /*pszPath*/, struct InfoSite *pSite) {
  ...
  if (!g_running.load()) {
    LogDebug("Starting streaming thread from Configure()");
    g_running = true;
    g_thread = std::thread(StreamingThread);
  }

  // Auto-fetch symbols and inject them via InfoSite
  int addedCount = 0;
  if (pSite && pSite->AddStockNew) {
    LogDebug("AddStockNew pointer is available. Fetching live data...");
    std::vector<DSEQuote> live;
    if (g_fetcher.FetchLive(live)) {
      LogDebug("FetchLive success. " + std::to_string(live.size()) +
               " symbols returned.");
      for (const auto &q : live) {
        if (pSite->AddStockNew(q.symbol)) {
          addedCount++;
        }
      }
      LogDebug("Successfully added " + std::to_string(addedCount) +
               " symbols via AddStockNew.");
      if (g_hMainWnd) {
        // Post update so AB refreshes symbol list
        PostMessage(g_hMainWnd, WM_USER_STREAMING_UPDATE, 0, 0);
      }
    } else {
      LogDebug("FetchLive failed: " + g_fetcher.LastError());
    }
  } else {
    LogDebug("pSite or pSite->AddStockNew is NULL");
  }

  char msgPtr[1024];
  sprintf_s(msgPtr,
            "BDShare DSE Data Feed v1.1.2\r\n\r\n"
            "Fetches live and historical data from www.dsebd.org.\r\n"
            "Auto-imported %d DSE active symbols.\r\n\r\n"
  ...
  MessageBoxA(g_hMainWnd, msgPtr, "BDShare DSE Plugin",
...
// ---------------------------------------------------------------------------
PLUGINAPI int GetQuotesEx(LPCTSTR ticker, int periodicity, int lastValid,
                          int size, Quotation *quotes,
                          GQEContext * /*context*/) {
  if (!ticker || !quotes || size <= 0)
    return 0;
  ...
  if (periodicity == PERIODICITY_EOD) {
    ...
    sprintf_s(startDate, "%04d-%02d-%02d", today.wYear - 2, today.wMonth,
              today.wDay);

    std::vector<DSEQuote> hist;
    if (!g_fetcher.FetchHistorical(symbol, startDate, endDate, hist)) {
      LogDebug("FetchHistorical failed for " + symbol);
      return lastValid + 1;
    }

    LogDebug("FetchHistorical fetched " + std::to_string(hist.size()) +
             " rows for " + symbol);

    std::sort(hist.begin(), hist.end(),
              [](const DSEQuote &a, const DSEQuote &b) {
                if (a.year != b.year)
    ...
    LogDebug("Returning " + std::to_string(n) + " historical quotes for " +
             symbol);
    return n;
  }
  ...
  // ── Intraday / real-time path ─────────────────────────────────────────
  {
    ...
    quotes[idx].DateTime =
        MakeAmiDate(q.year, q.month, q.day, q.hour, q.minute, q.second);
...
PLUGINAPI AmiVar GetExtraData(LPCTSTR pszTicker, LPCTSTR pszName,
  ...
  if (name == "BidPrice" || name == "AskPrice" || name == "BidVol" ||
      name == "AskVol") {
    DSEDepth dep = {};
    if (g_fetcher.FetchDepth(symbol, dep) && dep.levels > 0) {
      ...
      if (name == "BidPrice")
        ...
        val = (float)dep.buyVolume[0];
      else if (name == "AskVol")
        val = (float)dep.sellVolume[0];
