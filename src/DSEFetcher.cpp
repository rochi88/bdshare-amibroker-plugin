...
// HTTP GET
// ---------------------------------------------------------------------------
bool DSEFetcher::HttpGet( const std::wstring& host,
                           ...
                           std::string& body )
{
    ...
    HINTERNET hConn = WinHttpConnect( m_hSession, host.c_str(),
                                       INTERNET_DEFAULT_HTTP_PORT, 0 );
    if ( !hConn ) { m_lastError = "Connect failed"; return false; }

    std::wstring fullPath = path + ( query.empty() ? L"" : L"?" + query );
    HINTERNET hReq = WinHttpOpenRequest( hConn, L"GET", fullPath.c_str(),
...
// HTTP POST
// ---------------------------------------------------------------------------
bool DSEFetcher::HttpPost( const std::wstring& host,
                            const std::wstring& path,
...
float DSEFetcher::ParseFloat( const std::string& raw )
{
    ...
    for ( char c : raw )
        if ( c != ',' ) s += c;
    s = Trim( s );
    if ( s.empty() || s == "-" || s == "N/A" ) return 0.0f;
                                 ...
                                 std::vector<std::string>& cells )
{
    ...
    while ( pos < row.size() ) {
        ...
        if ( cellOpen == std::string::npos ) break;
        // Find </td>
        size_t tdEnd = row.find( "</td>", cellOpen );
        if ( tdEnd == std::string::npos ) tdEnd = row.find( "</TD>", cellOpen );
                                    ...
                                    std::string& tableHtml )
{
    ...
    while ( pos < html.size() ) {
        ...
        if ( tStart == std::string::npos ) return false;
        size_t tClose = html.find( '>', tStart );
        if ( tClose == std::string::npos ) return false;
                                   ...
                                   std::vector<DSEQuote>& out )
{
    ...
    while ( pos < tableHtml.size() ) {
        ...
        std::vector<std::string> cells;
        if ( !ExtractCells( row, cells ) || cells.size() < 11 ) continue;
                                    ...
                                    std::vector<DSEQuote>& out )
{
    ...
    std::string html;
    if ( !HttpPost( DSE_HOST, PATH_HIST, body, html ) )
        if ( !HttpPost( DSE_ALT_HOST, PATH_HIST, body, html ) )
            return false;
...
bool DSEFetcher::ParseDepthTable( const std::string& html, DSEDepth& out )
{
    ...
    while ( pos < html.size() && out.levels < 10 ) {
        ...
        std::vector<std::string> cells;
        if ( !ExtractCells( row, cells ) || cells.size() < 4 ) continue;
                                    ...
                                    std::vector<DSESectorEntry>& out )
{
    ...
    while ( pos < tableHtml.size() ) {
        ...
        std::vector<std::string> cells;
        if ( !ExtractCells( row, cells ) || cells.size() < 3 ) continue;

        DSESectorEntry e = {};
        strncpy_s( e.sector, cells[0].c_str(), 63 );
        e.change = ParseFloat( cells[1] );
        e.volume = ParseFloat( cells[2] );
        if ( cells.size() >= 4 ) e.value = ParseFloat( cells[3] );
        out.push_back( e );
    }
    return !out.empty();
}
